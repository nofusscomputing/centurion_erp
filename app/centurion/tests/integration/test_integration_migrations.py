...
class MigrationsTestCases:
    ...
    def test_run_migrations_upgrade(self, truncate_database_sql_string, run_command):
        ...
        #         "CENTURION_IMAGE_TAG=$(git rev-list -n 1 $(git describe --tags --abbrev=0)) "
        #         "docker-compose up -d centurion;'"
        #     ),
        #     description = 'set centurion image to latest git tagged image'
        # )
...
#                 "docker inspect -f '{{ index .Config.Image }}' centurion-erp | cut -d: -f2"
#             ),
#             description = 'fetch centurion-erp container tag, should latest git tag'
#         )
...
#         result = run_command(
#             command = 'docker exec -i centurion-erp python manage.py migrate',
#             description = 'perform upgrade migration'
#         )
