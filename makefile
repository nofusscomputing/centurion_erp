...
test-integration:
	...
	# if CENTURION_IMAGE_TAG=$(git log -1 --format=%H) docker-compose up -d; then
		...
		if ./setup-integration.sh; then
			...
			IS_INTEGRATION_TEST=true pytest --override-ini addopts= --no-migrations --reuse-db --tb=long --verbosity=2 --showlocals --junit-xml=integration.JUnit.xml app/*/tests/integration;
